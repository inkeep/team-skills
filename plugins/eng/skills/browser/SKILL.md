...
| GitHub PR screenshot | `pr-standard` | 1280×720 PNG | <200 KB | Crisp at GitHub's 894px display width. Upload via `/media-upload` skill for CDN URLs |
| GitHub PR GIF | `gif-compact` | 800×450 animated GIF | <10 MB | DPR 1 — GIF's 256-color palette is the bottleneck, not pixel density. Upload via `/media-upload` skill for CDN URLs |
| Video (internal or customer-facing) | `video` | 2560×1440 WebM → Bunny or Vimeo | — | Upload via `/media-upload` skill. Both platforms transcode to ABR. DPR 1 is correct for video |
