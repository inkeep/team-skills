...
# Design Assets file doesn't need to be open.
#
# Prerequisites:
...
#   ./launch-bridge.sh --both                   # opens Graphics Workspace + Design Assets, bridge on both
...
if [[ "${1:-}" == "--both" ]]; then
  ...
  FILE_NAMES=("Graphics Workspace" "Design Assets")
elif [[ -n "${1:-}" ]]; then
  FILES=("$1")
  FILE_NAMES=("$1")
else
...
if $BOTH; then
  ...
  echo "  Design Assets — for cross-file clone operations (fallback)"
else
  echo "Ready. Bridge is running in ${FILE_NAMES[0]}."
  echo "  Brand assets are imported via component keys — Design Assets file not needed."
fi
