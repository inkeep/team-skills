...
4. **Add connection elements last** — lines, arrows, and connectors depend on final positions. Create them after everything else is placed. **Use the connector recipes in `tools/figma-console.md`** — curved arrows use `vectorNetwork` with `tangentStart`/`tangentEnd` (NOT `vectorPaths` with bezier commands, which are unreliable). For circular flows, use the circular flow recipe with radius-perpendicular tangents. For hub-and-spoke, target node EDGES not centers. **Screenshot each connector individually** — a single sign error in a tangent vector reverses the curve, and batch creation without per-connector verification compounds errors. Verify connections visually — arrows should touch their target elements at the boundary, not float nearby or penetrate the center. **If any layout change happens after connectors are placed** (card resizing, repositioning, re-centering), **delete and rebuild all connectors from scratch** — adjusting individual connectors is error-prone and slower than a clean rebuild.
...
4. **Implement at least 2 improvements** — either elevating existing elements (Correct → Elevated) or adding new atoms from the additive scan (badges, texture layers, mockup chrome, connector details, accent elements). Fix the elements that have the most room to grow. Screenshot after each fix.
