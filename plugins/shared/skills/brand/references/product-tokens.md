...
Product UI tokens extracted from `agents-manage-ui` source code. Use these when creating product mockups (fidelity levels 1–3). For marketing tokens (the surround, headlines, backgrounds), use `manifest.md`.

> **Two-layer rule:** Inside the product mockup → these tokens. Outside the mockup → marketing tokens from `manifest.md`. See `references/product-representation.md` for the full framework.
